...
public class FurimaDao {
	...
	public List<FurimaDto> getList(int start, int end) {
		...
		String sql = "SELECT * FROM ( " + "  SELECT ROWNUM rnum, A.* FROM ( " + "    SELECT * FROM furima_posts "
				+ "    WHERE status='Y' ORDER BY post_id DESC " + "  ) A " + ") WHERE rnum BETWEEN ? AND ?";
	...
	public FurimaDto getPost(int postId) {
		...
		String sql = "SELECT * FROM furima_posts " + "WHERE post_id=? AND status='Y'";
	...
	public void insert(FurimaDto dto) {
		String sql = "INSERT INTO furima_posts " + "(post_id, user_id, title, description, price, image_path) "
	...
	public void delete(int postId) {
		...
		String sql = "UPDATE furima_posts SET status='N' WHERE post_id=?";
